...
{
	UINT64 id;
	float jointPos[JointType_Count * 4];
	float jointRots[JointType_Count * 4];
	int hands[2];

	void fillValues(IBody* pBody)
	{
		Joint joints[JointType_Count];
		JointOrientation rots[JointType_Count];
		...
		//fill in the hands
		hands[0] = leftHandState;
		hands[1] = rightHandState;

		HRESULT hr = pBody->GetJoints(_countof(joints), joints);
		//fill in pos
		for (int i = 0; i < JointType_Count; i++)
		{
			jointPos[4 * i + 0] = joints[i].Position.X ;
			jointPos[4 * i + 1] = joints[i].Position.Y;
			jointPos[4 * i + 2] = joints[i].Position.Z;
			jointPos[4 * i + 3] = joints[i].TrackingState;
		}
		hr =  pBody->GetJointOrientations(_countof(joints), rots);
		//fill in rot
		for (int i = 0; i < JointType_Count; i++)
		{
			jointRots[4 * i + 0] = rots[i].Orientation.x;
			jointRots[4 * i + 1] = rots[i].Orientation.y; 
			jointRots[4 * i + 2] = rots[i].Orientation.z; 
			jointRots[4 * i + 3] = rots[i].Orientation.w; 
		}
		//get the id
...
#define MAX_BODIES 6
SimpleBody gBody[MAX_BODIES];
//keeps track of how many bodies are actually tracked
int gNumTrackedBodies=0;

#include "PracticalSocket.h"
#include <iostream>           // For cout and cerr

char gDistUDPAddr[128];
int gDistUDPPort;

bool sendBody = false;
char gBodyUDPAddr[128];
int gBodyUDPPort;
...
void initUDP()
{
	...
	fscanf(file, "%s %d", &gDistUDPAddr, &gDistUDPPort);
	//do we have a second line?
	sendBody = fscanf(file, "%s %d", &gBodyUDPAddr, &gBodyUDPPort);
}
...
void sendData()
{
	...
	sock.sendTo(&gBodyDist, sizeof(int), gDistUDPAddr, gDistUDPPort);

	if (sendBody)
		sock.sendTo(&gBody[0], sizeof(SimpleBody)*gNumTrackedBodies, gBodyUDPAddr, gBodyUDPPort);
	//}
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
    if (m_hWnd)
    {
        ...
        if (SUCCEEDED(hr) && m_pRenderTarget && m_pCoordinateMapper)
        {
			...
			//set the number to 0 and then add on as we add data
			gNumTrackedBodies = 0;
            ...
            for (int i = 0; i < nBodyCount; ++i)
            {
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
						...
						if (sendBody)
							gBody[gNumTrackedBodies].fillValues(pBody);
						...
						//Kevin ADD
						gBodyDist = joints[JointType_SpineBase].Position.Z * 1000;
        ...
        WCHAR szStatusMessage[256];
		WCHAR addr[64];
		mbstowcs(addr, gBodyUDPAddr, 64);
		StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f    Time = %I64d \t Dist:%d \t Addr: %s \t Port:%d", 
			fps, (nTime - m_nStartTime), gBodyDist, addr, gBodyUDPPort);
